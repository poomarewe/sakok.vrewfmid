...
local function safeCarMovement(car, targetCF, duration, farmType)
    ...
    for step = 1, steps do
        ...
        local interpolatedCF = startCF:Lerp(targetRotation, alpha)
        
        -- Direct pivot (fastest method)
        car:PivotTo(interpolatedCF)
        
        -- Keep car upright (minimal overhead)
        primaryPart.AssemblyAngularVelocity = Vector3.zero
        
        task.wait(stepDuration)
    end
    
    -- Final snap ensures precision
    car:PivotTo(targetRotation)
    ...
    -- Check if we should continue after movement
    if not shouldContinue(farmType) then
    ...
    task.spawn(function()
        while shouldContinue() do
            local success, err = pcall(function()
                ...
                -- Enter race
                game:GetService("ReplicatedStorage").RemoteFunctions.RaceEvents.Entry:InvokeServer(
                    "Otsuki Death Trial",
                    "Circuit",
                    true,
                    {
                ...
                -- Vote to start
                game:GetService("ReplicatedStorage").RemoteFunctions.RaceEvents.VoteToStart:InvokeServer(
                    "Otsuki Death Trial",
                    "Circuit",
                    workspace["Proximity Triggers"]["Otsuki Death Trial"].Focus,
                    {
                ...
                repeat
                    ...
                    if tick() - guiWaitStart > 15 then
                        notify("Error", "Race GUI didn't appear", 3)
                        return
                    end
                until timeScoreLabel.Visible == true
                
                local originalText = timeScoreLabel.Text
                  -- Wait for race timer to start (number changes)
                ...
                while task.wait(0.03) do
                    ...
                    if tick() - raceStartTimeout > 15 then
                        notify("Error", "Race didn't start properly", 3)
                        return
                    end
                end
                ...
                repeat
                    ...
                    if tick() - carWaitStart > 10 then
                        notify("Error", "Car not found after race start", 3)
                        return
                    end
                until car
                  notify("Race Status", "Car found - preparing", 2)
                
                -- Set car primary part (minimal setup)
                if not car.PrimaryPart then
                ...
                if not shouldContinue() then return end                notify("Race", "Following path...", 2)
                
                -- Follow path with optimized timing (0.015s = ~16.4 seconds total for 1095 waypoints)
                ...
                for i = 1, #deathTrialPath do
                    ...
                    if i % 50 == 0 or i == #deathTrialPath then
                ...
                while not resultsFound and retryCount < maxRetries do
                    ...
                    if not resultsFound and timeout then
                        ...
                        -- Re-do the last 20 waypoints (faster retry timing)
                        notify("Race", "Re-doing final waypoints...", 2)
                        for i = startIndex + 1, totalWaypoints do
                            if not shouldContinue("death") then return end
                            local success = safeCarMovement(car, deathTrialPath[i], 0.018, "death")
                            if not success then break end
                        end
                    end
                end                -- Handle max retries reached - exit race and restart
                if not resultsFound then
                    notify("Max Retries", "Results never appeared - exiting race...", 3)
                      -- Reset velocities before exit
                    resetVelocities(car, player)
                ...
                -- Now ProgressGui has been found or timed out
                print("[RACE] ProgressGui check finished!")
            ...
            if not success then
                ...
                notify("Error", "Error occurred, retrying...", 3)
                task.wait(1)
            end
